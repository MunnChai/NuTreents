...
@export var explosion_range_component: GridRangeComponent
...
func _ready() -> void:
	actor = get_parent()
	if not explosion_range_component:
		explosion_range_component = Components.get_component(actor, GridRangeComponent)
	if not grid_pos_component:
		grid_pos_component = Components.get_component(actor, GridPositionComponent)
	
	range = explosion_range_component.get_tiles_in_range()
