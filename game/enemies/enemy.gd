...
func move_along_path() -> bool:
	...
	tween.tween_property(self, "position", global_pos, MOVE_DURATION)
...
func find_path_to_tree(tree: Twee) -> Array:
	...
	while (not to_visit.is_empty()):
		...
		var index_of_best: int = get_lowest_f_score(to_visit, paths, Vector2(target_pos)) 
...
func is_valid_tile(map_pos: Vector2i) -> bool:
	...
	# Do not go on Water
	var biome: int = terrain_map.get_cell_tile_data(map_pos).get_custom_data("biome")
...
func get_lowest_f_score(to_visit: Array[Vector2i], paths: Array[Array], target_pos: Vector2) -> int:
	
	var lowest_f_score: float = INF;
	var best_index: int = -1
	
	for i in range(0, to_visit.size() - 1):
		var g_score: int = paths[i].size();
		
		var map_pos = to_visit[i]
		var h_score: float = (Vector2(map_pos) - target_pos).length();
		
		var f_score = g_score + h_score
